...
export default function EditLessonLink({ params }: { params: { lessonId: string } }) {
  ...
  const fetchLesson = async () => {
    try {
      const response = await fetch(`/api/admin/lessons/${params.lessonId}`);
      ...
      if (data.success) {
        ...
        setError('Lesson link not found');
      }
    } catch (error) {
      setError('An error occurred while fetching the lesson link');
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setError(data.error || 'Failed to update lesson link');
      }
    } catch (error) {
      setError('An error occurred while updating the lesson link');
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="p-6">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-gray-900 mb-4">Lesson Link Not Found</h1>
          <p className="text-gray-600 mb-6">The lesson link you're trying to edit doesn't exist.</p>
          <Link
            href="/admin/lessons"
            className="bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 transition-colors"
          >
            Back to Lesson Links
          </Link>
        </div>
      </div>
    );
  ...
  return (
    <div className="p-6">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-gray-900">Edit Lesson Link</h1>
        <p className="text-gray-600">Update lesson link information and settings</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-end space-x-3">
          ...
          >
            {loading ? 'Updating...' : 'Update Lesson Link'}
          </button>
        </div>
      </form>
    </div>
